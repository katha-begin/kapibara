...
// Function to determine KPI color based on score relative to target
            ...
            if (isSingleProjectSelected) {
              return project.department.map((dept, index) => (
                <TableRow key={`${project.id}-${dept}-${index}`}>
                  ...
                  <TableCell className={cn("font-medium text-primary", index > 0 ? "border-t-0" : "")}>
                     {index === 0 ? project.name : ""}
                  </TableCell>
                  <TableCell className={cn("text-muted-foreground", index > 0 ? "border-t-0" : "")}>
                    {dept}
                  </TableCell>
                  <TableCell className={cn("text-right font-semibold", getKpiColor(project.kpiScore, targetKpi), index > 0 ? "border-t-0" : "")}>
                    {/* Display KPI as ratio */}
                    {(project.kpiScore / targetKpi).toFixed(2)}
                  </TableCell>
                  <TableCell className={cn(index > 0 ? "border-t-0" : "")}>
                   ...
                   <TableCell className={cn("text-right font-semibold text-primary", index > 0 ? "border-t-0" : "")}>
                    {project.mandays}
                  </TableCell>
                </TableRow>
              ));
            } else {
              // If multiple projects or 'all' projects are selected, show one row per project
              return (
